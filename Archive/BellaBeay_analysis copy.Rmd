...
<div style= "float:right;position: relative; top: +40px; relative; right: -4px; relative; left: +20px;">
```{r, echo = FALSE, warning = FALSE, cache=TRUE}

# load some original datasets to display
# files = c("minuteCaloriesNarrow_merged.csv", "minuteSleep_merged.csv" , "hourlyCalories_merged.csv", "hourlySteps_merged.csv")
# 
...
hourlyCalories <- read.csv(paste('./Fitbit_data/','hourlyCalories_merged.csv',sep = ''))
hourlySteps <- read.csv(paste('./Fitbit_data/','hourlySteps_merged.csv',sep = ''))
...
# find first two Ids
first_two_ids <- unique(minuteCalories$Id)[c(1,2)]

# display 
minuteCalories %>%
  group_by(Id) %>%
  filter(Id %in% first_two_ids, row_number() == c(1:5))  %>%
  kbl(align = "r", caption = 'MINUTE DATASET: minuteCaloriesNarrow_merged.csv',table.attr = "style='width:30%;'") %>%
  kable_paper("striped", full_width = F)%>%
...
minuteSleep %>%
  group_by(Id) %>%
  filter(Id %in% first_two_ids, row_number() == c(1:5))  %>%
  kbl(align = "r", caption = 'MINUTE DATASET: minuteSleep_merged.csv', table.attr = "style='width:30%;'") %>%
  kable_paper("striped", full_width = F) %>%
  scroll_box(width = "400px", height = "200px")
...
hourlyCalories %>%
  group_by(Id) %>%
  filter(Id %in% first_two_ids, row_number() == c(1:5))  %>%
  kbl(align = "r", caption = 'HOURLY DATASET: hourlyCalories_merged.csv',table.attr = "style='width:30%;'") %>%
  kable_paper("striped", full_width = F) %>%
  scroll_box(width = "400px", height = "200px")
...
hourlySteps %>%
  group_by(Id) %>%
  filter(Id %in% first_two_ids, row_number() == c(1:5))  %>%
  kbl(align = "r", caption = 'HOURLY DATASET: hourlySteps_merged.csv',table.attr = "style='width:30%;'")%>% kable_classic(full_width = F) %>%
  kable_paper("striped", full_width = F) %>%
  scroll_box(width = "400px", height = "200px")
